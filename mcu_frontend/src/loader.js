...
export default class Loader {
    ...
    async loadEvents(selectedSources) {
        try {
            console.log(
                `Fetching events for ${selectedSources.length} sources...`
            );
            ...
            this.data = [
                ...this.data,
                ...data.filter((d) => delta.includes(d.eid)),
            ];
            this.logDump
                .selectAll('div')
                .data(this.data)
                ...
                .text(
                    (el) =>
                        `(${el.filename} - ${el.line}), ${el.eid} ${el.date}`
                );
            console.log(
                `Added ${delta.length} events, total: ${
                    this.data.length
    ...
    unloadEvents(selectedSources, loadedSources) {
        ...
        console.log(
            `Removed ${delta.length} events, total: ${this.data.length}`
        );
        return this.data.length;
    }
