...
def main():
    ...
    (extr_events
        .addattr('characters', actions.s2__addattr__events__split_wikilinks, use_element=True, **{'valid': occ_chars})
        .addattr('characters_count', lambda elem: len(elem['characters']), use_element=True)
    ...
    Extractor(data=[ordered_sources_chars]).save('sources_chars')
